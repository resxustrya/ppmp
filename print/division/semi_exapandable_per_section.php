<?php
    ...
    $pdf->SetFont('Arial','',9);
    $st = $pdo->prepare("SELECT * FROM table_g WHERE created_by ='8888' GROUP BY item ORDER BY item ASC");
    $st->execute();
    ...
    foreach($items as $item):
        ...
        if(count($sections) > 0) {
            foreach($sections as $section):
                $st = $pdo->prepare("SELECT quantity from table_g WHERE item = ? AND created_by = ? LIMIT 1");
        ...
        $section_total = null;
    endforeach;
    $semi_expandable_total += $over_total;
    $grand_total += $over_total;
    $pdf->SetFont('Arial','B',10);
    $pdf->Row(array("","\t\t\t\t\t\t\t\t\t\t\t\t","","Total",number_format($over_total,2),"","",'','','','','','','',''));
    $pdf->Row(array("","SEMI-EXPANDABLE EQUIPEMENT AND FURNITURE TOTAL","","",number_format($semi_expandable_total,2),"","",'','','','','','','',''));
?>