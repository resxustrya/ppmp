...
$pdf->Row(array("","\t\t\t\t\t\t\t\t\t\t\t\tb. PER SECTION","","","","",'','','','','','','','Public Bidding',$division_name['source_fund'])); 


$pdf->SetFont('Arial','',9);
$st = $pdo->prepare("SELECT * FROM table_b WHERE created_by ='8888' GROUP BY item ORDER BY item ASC");
$st->execute();
...
foreach($items as $item):
    ...
    if(count($sections) > 0) {
        foreach($sections as $section):
            $st = $pdo->prepare("SELECT quantity from table_b WHERE item = ? AND created_by = ? LIMIT 1");
