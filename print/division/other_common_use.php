<?php
    ...
    include_once 'division/other_common_use_header.php';

   

    $st = $pdo->prepare("SELECT * FROM table_z GROUP BY item ORDER BY item ASC");
    $st->execute();
        ...
        foreach($items as $item):
            ...
            if(count($sections) > 0) {
                foreach($sections as $section):
                    $st = $pdo->prepare("SELECT quantity from table_z WHERE item = ? AND created_by = ? LIMIT 1");
