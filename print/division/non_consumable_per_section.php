<?php
    ...
    include_once 'division/non_consumable_header.php';

    $pdf->SetFont('Arial','',9);
    $st = $pdo->prepare("SELECT * FROM table_e WHERE created_by ='8888' GROUP BY item ORDER BY item ASC");
    $st->execute();
    ...
    foreach($items as $item):
        ...
        if(count($sections) > 0) {
            foreach($sections as $section):
                $st = $pdo->prepare("SELECT quantity from table_e WHERE item = ? AND created_by = ? LIMIT 1");
