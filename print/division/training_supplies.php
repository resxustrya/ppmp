<?php
    ...
    $pdf->Row(array("","\t\t\t\t\t\t\t\t\t\t\t\tc. TRAINING SUPPLIES","","","","",'','','','','','','','Public Bidding',$division_name['source_fund'])); 


    $pdf->SetFont('Arial','',9);
    $st = $pdo->prepare("SELECT * FROM table_c WHERE created_by ='8888' GROUP BY item ORDER BY item ASC");
    $st->execute();
    ...
    foreach($items as $item):
        ...
        if(count($sections) > 0) {
            foreach($sections as $section):
                $st = $pdo->prepare("SELECT quantity from table_c WHERE item = ? AND created_by = ? LIMIT 1");
